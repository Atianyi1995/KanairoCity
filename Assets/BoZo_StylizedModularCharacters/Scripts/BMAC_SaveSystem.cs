...
namespace Bozo.ModularCharacters
{
    ...
    public static class BMAC_SaveSystem
    {
        ...
        public static void SaveCharacter(OutfitSystem outfitSystem, string saveName, Texture2D icon = null) 
        {
            ...
            // Update main characterData asset if it exists
            if (outfitSystem.characterData != null)
            {
                var characterObject = outfitSystem.characterData as CharacterObject;
                if (characterObject != null)
                {
                    characterObject.data = data;
                    if (icon != null) characterObject.icon = icon;
                    #if UNITY_EDITOR
    ...
    #if UNITY_EDITOR
            // Only create new asset if it's not the primary one already linked
            string assetFullPath = "Assets/" + assetPath + "/" + saveName + ".asset";
            var existingAsset = AssetDatabase.LoadAssetAtPath<CharacterObject>(assetFullPath);
            ...
            if (existingAsset == null)
            {
                ...
                AssetDatabase.CreateAsset(CharacterSave, assetFullPath);
